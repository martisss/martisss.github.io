  ...
  function WobbleWindow(object, params) {
    ...
    function resizeCanvas() {
      ...
      if (settings.radius > 0) {
        //round,  ceil
        settings.pointCountX = Math.round(
          canvasContent.width / settings.radius
        );
        settings.pointCountY = Math.round(
          canvasContent.height / settings.radius
        );
      }
      if (settings.pointCountX % 2 == 0) {
      ...
      points.spaceX = Math.min(
        canvasContent.elementWidth,
        canvasContent.width / (settings.pointCountX - 1)
      );
      points.spaceY = Math.min(
        canvasContent.elementHeight,
        canvasContent.height / (settings.pointCountY + 1)
      );
      ...
      if (settings.movementRight) settings.canvasWidth += points.radius;
      ...
      if (settings.movementBottom) settings.canvasHeight += points.radius;
    ...
    function addWindow() {
      ...
      for (i = 0, l = settings.pointCountX; i < l; i++) {
        if (settings.movementTop) {
          if (flag) {
            point = addPoint(
              canvasContent.left + i * points.spaceX,
              canvasContent.top,
              0,
              0,
              0,
      ...
      for (i = 1, l = settings.pointCountY + 1; i < l; i++) {
        if (settings.movementRight) {
          if (flag) {
            point = addPoint(
              canvasContent.left + canvasContent.width,
              canvasContent.top + i * points.spaceY,
              0,
              0,
              0,
              true,
          ...
          if (i === 1) {
            point = addPoint(
              canvasContent.left + canvasContent.width,
              canvasContent.top + (i - 1) * points.spaceY,
              0,
              0,
              0,
              false,
      ...
      for (i = settings.pointCountX - 1, l = -1; i > l; i--) {
        if (settings.movementBottom) {
          if (flag) {
            point = addPoint(
              canvasContent.left + i * points.spaceX,
              canvasContent.top + canvasContent.height,
              0,
              0,
              0,
              true,
      ...
      for (i = 1, l = windowPoints.length; i < l; i += 2) {
        ...
        if (point.distance < point.radius) {
          if (settings.moveTypeIn === "wobble") {
            point.sx =
              point.sx * settings.wobbleFactor +
              (mousePos.x - point.x) * settings.wobbleSpeed;
            point.sy =
              point.sy * settings.wobbleFactor +
              (mousePos.y - point.y) * settings.wobbleSpeed;
          ...
          if (settings.moveTypeOut === "wobble") {
            point.sx =
              point.sx * settings.wobbleFactor +
              (point.xp - point.x) * settings.wobbleSpeed;
            point.sy =
              point.sy * settings.wobbleFactor +
              (point.yp - point.y) * settings.wobbleSpeed;
            point.x = point.x + point.sx;
            point.y = point.y + point.sy;
      ...
      return this.each(function () {
        if (!$.data(this, "plugin_WobbleWindow")) {
          ...
          if (plugin[params]) {
            ...
            $.error(
              "Method " + params + " does not exist on jQuery.wobbleWindow"
            );
          }
        }
      });
